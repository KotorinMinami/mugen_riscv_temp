...
class QemuVM(object):
    def __init__(self,id=1,port=12055,user='root',password='openEuler12#$',vcpu=4,memory=4,
                 ...
                 bkfile='openeuler-qemu.qcow2' , path='/root/GitRepo/mugen-riscv' , gene=False , disk=1):
        self.id = id
        self.port = port
        self.ip = '127.0.0.1'
        self.user = user
        ...
        if disk > 1:
            for i in range(1 , disk):
                cmd = 'qemu-img create -f qcow2 '+self.workingDir+"disk"+str(id)+str(i)+'.qcow2 500M'
                res = os.system(cmd)
                if res != 0:
    ...
    def start(self):
        ...
        if self.disk > 1:
            for i in range(1 ,self.disk):
                cmd += "-drive file=disk"+str(self.id)+str(i)+".qcow2,format=qcow2,id=hd"+str(i)+" -device virtio-blk-device,drive=hd"+str(i)+" "
    ...
    def rm_disk(self):
        for i in range(1 , self.disk):
            os.system('rm -f '+ self.workingDir+'disk'+str(self.id)+str(i))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-a',type=int,default=0,help='add the disk for test')
    ...
    if args.list_file is not None:
        ...
        maxdisk = 0
        for testsui in test_target.test_list:
            mugen_file = open(test_env.suite_cases_path+testsui+".json",'r')
            mugen_data = json.loads(mugen_file.read())
            maxdisk = max(max(mugen_data.get('add disk' , [1])),maxdisk)
        print('maxdisk = '+str(maxdisk))
        qemuVM = []
        for i in range(args.x):
            qemuVM.append(QemuVM(i,ports[i],vcpu=args.c,memory=args.M,workingDir=args.w,bkfile=args.b,path=args.d.rstrip('/'),gene=args.generate,disk=maxdisk))   
        targetQueue = Queue()
